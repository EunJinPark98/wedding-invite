 ...
 *
 * 임시 저장 사진을 정기 청소가 지우는 기준이기도 하다. 사진은 "사진 추가"를
 * 누른 순간 올라가지만 초대장은 "제작하기"를 눌러야 저장되므로, 만들다 만
 * 사람의 사진은 어느 초대장에도 안 딸린 채 남는다. 이어서 쓸 수 있는 동안
 * 그 사진을 지워 버리면 돌아와서 이어서 쓰기를 눌렀을 때 사진이 깨진다.
